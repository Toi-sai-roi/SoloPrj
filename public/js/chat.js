...
function getTopReactions(reactionMap, reactionTimestamps = {}) { // hàm mới để lấy 3 reaction phổ biến nhất dựa trên reactionMap và reactionTimestamps
  const emojiStats = {};
  for (const [userId, emoji] of Object.entries(reactionMap)) {
    ...
    const ts = reactionTimestamps[userId] || 0;
    if (ts > emojiStats[emoji].lastTimestamp) {
      emojiStats[emoji].lastTimestamp = ts;
    }
  }

  const sorted = Object.entries(emojiStats)
    .sort((a, b) => b[1].lastTimestamp - a[1].lastTimestamp)
    .slice(0, 3);

  return sorted.map(([emoji, stats]) => ({ emoji, count: stats.count }));
}
...
function updateMessageReaction(messageId, reactionMap, reactionTimestamps) { // hàm mới để cập nhật giao diện reactions khi có thay đổi
  const messageDiv = document.querySelector(`.msg-wrapper[data-message-id="${messageId}"]`);
  if (!messageDiv) return;

  let reactionsRow = messageDiv.querySelector('.reactions-row');
  if (!reactionsRow) {
    reactionsRow = document.createElement('div');
    reactionsRow.className = 'reactions-row';
    const msgTime = messageDiv.querySelector('.msg-time');
    if (msgTime) {
      msgTime.insertAdjacentElement('afterend', reactionsRow);
    } else {
...
function appendChatMessage(msg) {
  ...
  if (msg.reactions) {
    ...
    const reactionTimestamps = msg.reaction_timestamps || {};
