...
// v9.1-fix: #12 import từ lib/broadcast thay vì server.js
// ==========================================
...
const { broadcastStatusChangeToPair } = require('../lib/broadcast');
...
router.post('/block', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      await client.query(`DELETE FROM friends WHERE user1 = $1 AND user2 = $2`, [u1, u2]);

      await client.query('COMMIT');

      broadcastStatusChangeToPair(blocker, target);

      res.json({ success: true, message: `Network node ${target.toUpperCase()} isolated` });
...
router.post('/unblock', authenticateToken, async (req, res) => {
  try {
    ...
    if (!target) return res.status(400).json({ error: 'Invalid data' });

    await query(`DELETE FROM blocks WHERE blocker = $1 AND blocked = $2`, [blocker, target]);
