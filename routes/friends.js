...
router.get('/status/:target', authenticateToken, async (req, res) => {
  try {
    ...
    const imBlocking = await query(`
      SELECT 1 FROM blocks WHERE blocker = $1 AND blocked = $2
    `, [me, target]);

    if (imBlocking.rows.length > 0) {
      return res.json({ relation: 'blocking' });
    }

    const theyBlocking = await query(`
      SELECT 1 FROM blocks WHERE blocker = $1 AND blocked = $2
    `, [target, me]);

    if (theyBlocking.rows.length > 0) {
      return res.json({ relation: 'blocked_by' });
    }
