...
function broadcastToUser(targetUsername, payload) { // Gửi payload đến tất cả các kết nối WebSocket của targetUsername
...
function broadcastStatusChange(username, online) { // Gửi thông báo trạng thái online/offline của username đến tất cả người dùng đang online
...
function broadcastStatusChangeToPair(userA, userB) { // Gửi thông báo trạng thái online/offline của userA đến userB và ngược lại, nếu cả hai đang online
  if (!_onlineUsers) return;
...
function broadcastSystemMessage(text) { // Gửi thông báo hệ thống đến tất cả người dùng đang online
  if (!_onlineUsers) return;
